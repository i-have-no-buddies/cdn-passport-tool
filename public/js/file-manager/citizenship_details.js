function updateFolder(elem, folder) {
  bootbox.confirm({
    title: 'Update Folder',
    message: `Do you really want to update the folder <i>"${folder}"</i>?`,
    closeButton: false,
    buttons: {
      confirm: {
        label: 'Yes',
    ...
    callback: function (result) {
      if (result) {
        $('#' + elem + ' > input').val($('#inpFolderName').val());
        $('#' + elem).submit();
      }
    },
  });
...
function deleteSubFolder(elem, folder) {
  bootbox.confirm({
    title: 'Delete Folder',
    message: `Do you really want to delete the sub folder <i>"${folder}"</i>?`,
    closeButton: false,
    buttons: {
      confirm: {
        label: 'Yes',
...
$(document).ready(function () {
  ...
  $('input[name="subfoldername"]').on('keyup', function () {
    let text = $(this).val();
    text = text.replace(/[^a-zA-Z0-9\-\_ ]/g, '');
    $(this).val(text);
  });
  $('input[name="subfoldername"]').on('change', function () {
    let text = $(this).val();
    text = text.replace(/[^a-zA-Z0-9\-\_ ]/g, '');
    $(this).val(text);
  });
